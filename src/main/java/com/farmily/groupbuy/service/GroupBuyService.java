...
public class GroupBuyService {
	...
	public void ship(Integer orderId, Integer farmerId) {
	    GroupBuyOrderVO order = groupBuyOrderRepository
	            .findByOrderIdAndGroupBuyId_Product_FarmerId(orderId, farmerId)
	...
	public List<ShowJoinedGroupBuyDTO> showJoinedGroupBuy(Integer userId) {
		...
		return list.stream()
				.map(joined -> new ShowJoinedGroupBuyDTO(joined.getGroupBuyId().getStatus(),
						joined.getGroupBuyId().getDdlDatetime(), joined.getGroupBuyId().getPickupAddress(),
						joined.getGroupBuyId().getProduct().getProductName(), joined.getBuyQty(),
						joined.getPaidAmount()))
				.toList();
	}
	...
	public GroupBuyParticipationDTO showGroupBuyProgress(Integer groupBuyId,Integer farmerId) {
	    GroupBuyVO groupBuy = repository
	            .findByGroupBuyIdAndProduct_FarmerId(groupBuyId, farmerId)
	            .orElseThrow(() -> new RuntimeException("查無此團購或你沒有權限"));
